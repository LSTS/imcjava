...
public class ExtendedDeviceUpdate extends IridiumMessage {
    ...
    public Collection<IMCMessage> asImc() {
        ...
        for (Position pos : positions.values()) {
            ...
            sensorInfo.setSensorClass(DeviceUpdate.getSystemType(pos.id));
            msgs.add(sensorInfo);
        }
    ...
    public String toString() {
        ...
        for (Position p : positions.values()) {
            s += "\t("+IMCDefinition.getInstance().getResolver().resolve(p.id)+", "+p.posType+") --> "+ Math.toDegrees(p.latRads)+", "+ Math.toDegrees(p.lonRads)+"\n";
        }
        return s;         
    }
